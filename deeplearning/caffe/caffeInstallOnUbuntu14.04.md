...
(2) `百度网盘`   
链接：http://pan.baidu.com/s/1qYHTtwC 密码：80wq  
------------------------------------------分割线------------------------------------------    
...
export PATH
```
保存后执行命令`source /etc/profile`使之生效。同时创建以下文件:   
```
sudo vim /etc/ld.so.conf.d/cuda.conf
# 内容是
/usr/local/cuda/lib64
```
保存后，执行命令`sudo ldconfig`使之生效。   

## 4. 安装`CUDA Sample` 及 `ATLAS`   
